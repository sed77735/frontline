...
//團隊原本資料------------------------------
$sql = "SELECT * FROM `team` WHERE `sid`=". intval($_GET['sid']) ;
$rs = $mysqli->query($sql);
$row = $rs->fetch_assoc();
...
$_SESSION['team_info'] = $row;


//SELECT m.name as mname FROM `members` m JOIN `team` t ON m.team_sid = t.sid WHERE t.sid = 1 AND m.sid NOT IN (2)

//原有隊員列表-------------------------------
$tm_sql = sprintf("SELECT m.name as mname, m.sid as msid FROM `members` m JOIN `team` t ON m.team_sid = t.sid 
                  WHERE t.sid =". $_GET['sid'] . " AND m.sid NOT IN (".$_SESSION['user']['sid'] .")" );
//echo $tm_sql;
...
//表格-------------------
//if(!empty($_SESSION['user']) and !empty($_SESSION['team_img']) and !empty($_SESSION['team_bg_img'])) {
...
//        rename(iconv('UTF-8','big5',$target_bg),iconv('UTF-8','big5', $dir . $file_title_bg . '_bg_' . $_POST['code'] . '.' . $file_name_bg));
        if(!empty($_SESSION['team_img']) and !empty($_SESSION['team_bg_img'])) {

            $sql = sprintf("UPDATE `team` SET 
            `name` = '%s', `code` = '%s', `public` = '%s', 
            `image` = '%s', `bg_image` = '%s', 
            `color` = '%s', `intorduction` = '%s', `motto` = '%s'
            WHERE `sid` = '%s'",
...
//            exit;
        ...
        if(!empty($_SESSION['team_img'])) {
            ...
            WHERE `sid` = '%s'",
                $_POST['name'], $_POST['code'], $_POST['public'],
                $_SESSION["team_img"]["team_file"]["name"], $_SESSION["team_info"]["bg_image"],
                $_POST['color'], $_POST['intorduction'], $_POST['motto'],
                $_GET['sid']);
        } elseif(!empty($_SESSION['team_bg_img'])) {
            $sql = sprintf("UPDATE `team` SET 
            `name` = '%s', `code` = '%s', `public` = '%s', 
            `image` = '%s', `bg_image` = '%s', 
            `color` = '%s', `intorduction` = '%s', `motto` = '%s'
            WHERE `sid` = '%s'",
                $_POST['name'], $_POST['code'], $_POST['public'],
                $_SESSION["team_info"]["image"], $_SESSION["team_bg_img"]["team_bg_file"]["name"],
                $_POST['color'], $_POST['intorduction'], $_POST['motto'],
                $_GET['sid']);
        }
//            echo $sql;
...
//            $team_sid = $mysqli->insert_id;
            ...
            //如現有隊員被刪除 更新現有隊員 team_sid 為 0
            if (isset($_SESSION['team_original_members'])) {
                $sids = array_keys($_SESSION['team_original_members']);
                $m_sql = sprintf("UPDATE `members` SET `team_sid` = '%s' WHERE `sid` IN (%s)", 0, implode(',', $sids));
                $mysqli->query($m_sql);
            }
...
//            $m_sql = sprintf("UPDATE `members` SET `team_sid` = '%s' WHERE `sid` = '%s'",
//                $team_sid, $_SESSION['user']['sid'] );
//        echo $m_sql;

//            $success = $mysqli->query($m_sql);
...
//                $_SESSION['user'] = $row;
//            }
            unset($_SESSION['team_original_members']);
            unset($_SESSION['team_members']);
            if (! isset($_SESSION['create_team'])){
                $_SESSION['create_team'] = 2;
            }
            header("Location: team_info.php?sid=".$_GET['sid']);
            ...
            <form class="flexbox" name="form1" method="post" onsubmit="return checkForm();">
                <div class="top_info flexbox">
                    <div class="img_upload_group flexbox">
                        <div class="form-group img_upload_groupleft">
                            ...
                            <div class="img_upload_review">
                                <img src="<?= empty($image) ? "image/add_member_photo.png" : 'team_img/'.$code."/". $image ?>" alt="">
                            </div>
                            <div class="img_upload">
                                <div class="custom_img team_img flexbox">
                                    <span id="img_name" onkeyup="imgname(this.id)"><?= $image ?></span>
                                    <p>檔案上傳</p>
                                </div>
                                <input class="img_file" type="file" name="team_file" id="team_file"><br>
                            </div>
                            <div class="img_upload_info flexbox form_error">
                        ...
                        <div class="form-group img_upload_groupright">
                            <div class="form-group">
                                ...
                                <div class="name_text_info flexbox">
                                    <input type="text" class="form-control" id="name" name="name" value="<?= $name ?>" onkeyup="testName(this.name)">
                                    <i class="fa fa-info-circle fa-2x form_error"></i>
                                    <span class="info form_error">請填寫團隊名稱</span>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="teamcode">團隊代號</label>
                                <div class="teamcode_text_info flexbox">
                                    <input type="text" class="form-control" id="teamcode" name="code" value="<?= $code ?>" onkeyup="upperCase(this.id)">
                                    <i class="fa fa-info-circle fa-2x form_error"></i>
                                    <span class="info form_error">請輸入1大寫英文字母+3位數字</span>
                                </div>
                            </div>
                            <div class="form-group">
                                <section class="team_open_section">
                                    <div class="team_open_select">
                                        <?php if ($public=='private'): ?>
                                        <input type="checkbox" class="check_item" name="public" value="private" id="checkbox-1" checked>
                                        <label for="checkbox-1"><span class="checkbox">僅限邀請參加的私人團隊</span></label>
                                        <input type="checkbox" class="check_item" name="public" value="public" id="checkbox-2">
                    ...
                    <div class="img_upload_groupbk flexbox">
                        <div class="form-group img_upload_groupbk_left">
                            ...
                            <div class="img_upload_groupbk_review">
                                <img src="<?= empty($bg_image) ? "image/add_member_photo.png" : 'team_img/'.$code."/".$bg_image ?>" alt="">
                            </div>
                            <div class="img_upload">
                                <div class="custom_img bg_img flexbox">
                                    <span id="bg_img_name"><?= $bg_image ?></span>
                                    <p>檔案上傳</p>
                                </div>
                        ...
                        <div class="form-group img_upload_groupbk_right">
                            <div class="form-group1">
                                <div class="img_upload_groupbk_info flexbox form_error">
                                    <i class="fa fa-info-circle fa-2x"></i>
                                    <span class="info">
                                                請選擇一張背景圖<br/>
                                                您可自訂上傳個人圖片<br/>
                            ...
                            <div class="form-group">
                                <div class="img_upload_color_radio">
                                    <ul>
                                        <li>
                                            <input type="radio" id="a-option" name="color" value="rgba(255,0,46,.2)">
                    ...
                    <div class="team_intorduction flexbox">
                        ...
                        <textarea id="team_intorduction" name="intorduction" cols="30" rows="10" placeholder="最多80字唷"><?= $intorduction ?></textarea>
                    </div>
                    <div class="team_slogan flexbox">
                        <label for="team_slogan">團隊座右銘</label>
                        <textarea id="team_slogan" name="motto" cols="30" rows="10" placeholder="最多20字唷"><?= $motto ?></textarea>
                    </div>
...
<!--                                            <td id="remove_btn" class="memberlist_delete">剔除</td>-->
                                        ...
                                        <tbody id="original">
                                        <?php while ($tm_row = $tm_result->fetch_assoc()): ?>
                                        <tr id="">
                                            <td class="member_name memberlist_username_id"><?= $tm_row['mname'] ?></td>
                                            <td class="memberlist_delete remove_original_btn" data-sid="<?= $tm_row['msid'] ?>">剔除</td>
                                        </tr>
                                        <?php endwhile; ?>
...
<!--                                        </tr>-->
                        ...
                        <div class="btn flexbox">
                            <a class="btn1" href="single_activity.php">返回團隊介紹</a>
...
//        }
        ...
        $.get('aj_check_code_for_manage.php', {code: code, ori_code:ori_code}, function(data){
...
//          alert('123');
    ...
    //移除原有隊員-----------------------
    $('#remove_member').on('click','.remove_original_btn',function(){
...
//            alert('商品已加入購物車');
        tr.remove();
        $.get('remove_original_to_team.php', {sid:sid, name:name}, function(data){
